...
prepare_for_join = function(input_data, clmn_name) {
  input_data %>% 
    select(EVTYPE, combined_col = !!as.symbol(clmn_name)) %>%
...
unique_max = function(input_data) {
  input_data %>% 
    aggregate(combined_col ~ EVTYPE, ., max) %>%
...
PROPDMG_by_state1 <- by_state(storm_data_csv, "PROPDMG")
CROPDMG_by_state1 <- by_state(storm_data_csv, "CROPDMG")
...
```{r health_plot}
ggplot(data=new_data_health, aes(x = combined_col, y = EVTYPE, fill = dmg_type)) +
  geom_bar(stat = "identity", position = position_dodge()) +
  geom_text(aes(label = combined_col), vjust = 1.6, color = "white",
...
ggplot(data = new_data_econ, 
       aes(x = combined_col, y = EVTYPE, fill = dmg_type)) +
  geom_bar(stat = "identity", position = position_dodge()) +
  geom_text(aes(label = combined_col), vjust = 1.6, color = "white",
            position = position_dodge(0.9), size = 3.5) +
